...
class AdminInviteRequestsApproveApplicationJSON:
    ...
    invite_request_id: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('invite_request_id') }})
    r"""ID of the request to invite."""
    ...
    r"""ID for the workspace where the invite request was made."""
    

@dataclasses.dataclass
class AdminInviteRequestsApproveRequest:
    
    request_body: AdminInviteRequestsApproveApplicationJSON = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})
    token: str = dataclasses.field(metadata={'header': { 'field_name': 'token', 'style': 'simple', 'explode': False }})
    r"""Authentication token. Requires scope: `admin.invites:write`"""
